...
namespace Aima.API.Controllers.Admin
{
    ...
    public class AdminMasterController : CustomBaseController
    {
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> SaveUserDesignation(UserDesignation_Request parameters)
        {
            int result = await _adminMasterRepository.SaveUserDesignation(parameters);
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GetUserDesignationList(BaseSearchEntity parameters)
        {
            IEnumerable<UserDesignation_Response> lstRoles = await _adminMasterRepository.GetUserDesignationList(parameters);
            _response.Data = lstRoles.ToList();
            _response.Total = parameters.Total;
            return _response;
        }
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GetUserDesignationById(int Id)
        {
            if (Id <= 0)
            {
                _response.Message = "Id is required";
            ...
            {
                var vResultObj = await _adminMasterRepository.GetUserDesignationById(Id);
                _response.Data = vResultObj;
            }
            return _response;
        }
